...
$json = file_get_contents("liste_voyage.json");
$treks = json_decode($json, true);

$filtre_region = $_GET["region"] ?? "";
$filtre_date = $_GET["date"] ?? "";
$filtre_difficulte = $_GET["difficulte"] ?? "";
$filtre_tarif_max = $_GET["tarif_max"] ?? "";

$recherche_active = !empty($filtre_region) || !empty($filtre_date) || !empty($filtre_difficulte) || !empty($filtre_tarif_max);
...
function afficherTrek($trek) {
    ...
    echo "<br><a href='" . htmlspecialchars($trek["lien"]) . "' class='details-link'>Voir plus</a>";
    echo "</div>";
    echo "</div>";
}
...
<div class="filtre">	
    <div class="contenu">
	 ...
	 <form action="filtre.php" method="GET">
            ...
            <input type="number" name="tarif_max" id="filtre-tarif" placeholder="Ex: 500€" value="<?= htmlspecialchars($filtre_tarif_max) ?>">
	...
	<?php
            ...
            foreach ($treks as $trek) {
                ...
                $match_tarif = empty($filtre_tarif_max) || $trek["tarif"] <= $filtre_tarif_max;
